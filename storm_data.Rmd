...
dim(storm)
```
Now, there are 751740 rows and 38 columns in total.
...
sum.FATALITIES.Event <- aggregate(FATALITIES~EVTYPE,stormData,sum)
sum.fat.Event.sort <- sum.FATALITIES.Event[order(-sum.FATALITIES.Event$FATALITIES),]
fatalities <- sum.fat.Event.sort[1:20,]

sum.inj.Event <- aggregate(INJURIES~EVTYPE,stormData,sum)
sort.inj <- sum.inj.Event[order(-sum.inj.Event$INJURIES),]
injuries <- sort.inj[1:20,]
...
property.damage <- property.damage.order [1:20,]

corp.damage.sum <- aggregate(cropDamage~EVTYPE,storm.Convertion,sum)
corp.damage.order <-corp.damage.sum[order(-property.damage.sum$propertyDamage),]
corp.damage <- corp.damage.order [1:20,]
...
```{r,echo = TRUE}
propertyPlot <- qplot(EVTYPE, data = property.damage, weight = propertyDamage, geom = "bar", binwidth = 1) + 
   geom_bar(fill="red" )+
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_y_continuous("Property Damage in US dollars")+ 
    xlab("Severe Weather Type") + ggtitle("Total Property Damage by\n Severe Weather Events in\n the U.S. from 1995 - 2011")

cropPlot<- qplot(EVTYPE, data = crop.damage, weight = cropDamage, ) + 
